...
if __name__ == "__main__":
    ...
    nu = nu_s[-1]  # For nu=inf, the kernel becomes equivalent to the RBF kernel

    kernel = (ConstantKernel(constant_value=1.0, constant_value_bounds=(0.01, 10.0))
              * Matern(nu=nu_s[2], length_scale=0.01, length_scale_bounds=(1e-2, 1e3))
    ...
    gp = GaussianProcessRegressor(kernel=kernel, n_restarts_optimizer=50,
                                  random_state=42, normalize_y=True, alpha=1e-10)
    ...
    print(f"marginal log likelihood: {pipe[1].log_marginal_likelihood_value_}")
    print(f"R2 (coefficient of determination): {pipe.score(x_test, y_test)}")
    ...
    plotter = PlotGPR(f"GPR with {pipe[1].kernel_}", "Time", "Water Demand", 2.0)
    plotter.plot(x_indexes_train, y_train, x_indexes_test, y_test, x_indexes, y_mean, y_cov, x_ticks, x_labs_plot)
